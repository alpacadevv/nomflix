...
interface IDetailPresenterProps {
  result: IContentDetail | null;
  error?: string;
  loading: boolean;
}

const DetailPresenter: React.FC<IDetailPresenterProps> = ({
  result,
  error,
  loading,
}) => (
  loading ? (
    ...
    <Container>
      <Helmet>
        <title>{`${result && (result.original_title ? result.original_title : result.original_name)} | Nomflix`}</title>
      </Helmet>
      <Backdrop
        bgImage={
